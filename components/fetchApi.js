...
class FetchFromApi {
    ...
    async getBrand() {
        try {
          let response = await fetch("https://www.carboninterface.com/api/v1/vehicle_makes", {
            method: "GET",
            headers: {
    ...
    async getModel(id) {
        try {
            let response = await fetch(`https://www.carboninterface.com/api/v1/vehicle_makes/${id}/vehicle_models`, {
              method: "GET",
              headers: {
                Authorization: apiKey,
                "Content-Type": "application/json",
